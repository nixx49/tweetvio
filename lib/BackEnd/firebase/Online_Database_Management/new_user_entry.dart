...
class CloudStoreDataManagement {
  final _collectionName = 'tweetvio_users';
  ...
  Future<bool> checkThisUserAlreadyPresentOrNot(
    ...
    try {
      ...
      print(
          'Error in Checked This User Already Present or not:${e.toString()}');
      return false;
    }
  }
  ...
  Future<List<Map<String,dynamic>>> getAllUsersListExceptMyAccount({required String currentUserEmail}) async {
    try {
      ...
      print('Error in get All User List: ${e.toString()}');
      return [];
    }
  }

    Future<Map<String,dynamic>?> _getCurrentAccountAllData({required String email}) async {
      try{
  ...
  Future<List<dynamic>> currentUserConnectionRequestList({required String email}) async {
    try{
  ...
  fetchRealTimeDateFromFirestore() async{
    try{
